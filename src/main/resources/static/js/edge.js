...
// across every OS) down into one row per Edge major version. Unordered — the table sorts.
export function aggregateByVersion(rawUserAgents, humanStats) {
    ...
    for (const row of rawUserAgents) {
        const version = edgeMajorVersion(row.name);
...
// Sort keys for the Edge Versions table — each maps a row to the numeric value to compare on,
// so "Edge Version" sorts by the version number itself, never lexicographically on label text
// (which would put "Edge 10" before "Edge 9").
const SORT_VALUE = {
    ...
    tbody.innerHTML = rows.map((row, i) => {
        ...
        return `
            <tr>
                ...
                <td class="font-monospace">Edge ${row.version}</td>
