...
final class ResultTypeSql {
    ...
    // RefreshHit is CloudFront re-validating a still-fresh cached object with the origin — a cache
    // hit from the visitor's perspective, so it is counted as a Hit everywhere.
    static final String HIT_TYPE_LIST = "'Hit','RefreshHit'";
    ...
    static String resultTypeSums(String tableAlias) {
        ...
        return """
                SUM(CASE WHEN %1$sedge_response_result_type IN (%3$s) THEN 1 ELSE 0 END) as hit,
                ...
                """.formatted(p, FUNCTION_TYPE_LIST, HIT_TYPE_LIST);
    }
    ...
    static String resultTypeFlags(String tableAlias) {
        String p = tableAlias.isEmpty() ? "" : tableAlias + ".";
        return """
                CASE WHEN %1$sedge_response_result_type IN (%3$s) THEN 1 ELSE 0 END as hit,
